...
const getSql = () => {
  if (!_sql) {
    ...
    if (!url) {
      throw new Error("Critical Error: DATABASE_URL or POSTGRES_URL environment variable is missing. Please set it in your Vercel Dashboard.");
    }
    _sql = neon(url);
  }
  return _sql;
...
export async function initDb() {
  const url = process.env.DATABASE_URL || process.env.POSTGRES_URL;
  if (!url) {
    console.error("DATABASE_URL or POSTGRES_URL not found. Connection cannot be established.");
    return;
  }
